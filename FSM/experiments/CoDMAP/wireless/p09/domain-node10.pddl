...
(:action generate-data_node10
	...
	:effect (and
		(not (energy node10 ?e0))
		(energy node10 ?e1)
		(has-data node10 node10)
	)
)


(:action add-to-message_node10
	:parameters (?s2 - sensor ?m - message)
	:precondition (and
		(has-data node10 ?s2)
		(is-message-at ?m node10)
		(not (message-data ?m ?s2))
	)
	:effect (and
		(not (has-data node10 ?s2))
		(message-data ?m ?s2)
	)
)


(:action send-message_node10
	:parameters (?receiver - node ?m - message ?e0 - level ?e1 - level)
	:precondition (and
		(energy node10 ?e0)
		(higher ?e0 Zero)
		(next ?e0 ?e1)
		(neighbor node10 ?receiver)
		(is-message-at ?m node10)
		(not (is-message-at ?m ?receiver))
		(not (sending node10 ?receiver ?m))
	)
	:effect (and
		(not (energy node10 ?e0))
		(not (is-message-at ?m node10))
		(energy node10 ?e1)
		(sending node10 ?receiver ?m)
	)
)


(:action receive-message_node10
	:parameters (?sender - sensor ?m - message)
	:precondition (and
		(not (is-message-at ?m node10))
		(sending ?sender node10 ?m)
	)
	:effect (and
		(not (sending ?sender node10 ?m))
		(is-message-at ?m node10)
	)
)


(:action get-data-from-message_node10
