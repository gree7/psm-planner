...
EagerSearch::EagerSearch(
    ...
    : SearchEngine(opts),
      ...
      open_list(opts.get<OpenList<pair<state_var_t *, short *> > *>("open")),
...
void EagerSearch::initialize() {
    ...
    if (open_list->is_dead_end()) {
        ...
        open_list->insert(make_pair(node.get_state_buffer(), node.get_state_buffer_plans()));
    }
}
...
int EagerSearch::step() {
    ...
    for (int i = 0; i < applicable_ops.size(); i++) {
        ...
        if (succ_node.is_new()) {
            ...
            open_list->insert(make_pair(succ_node.get_state_buffer(), succ_node.get_state_buffer_plans()));
...
pair<SearchNode, bool> EagerSearch::fetch_next_node() {
    ...
    while (true) {
        ...
        pair<state_var_t *, short *> pair = open_list->remove_min(
                        use_multi_path_dependence ? &last_key_removed : 0);
        State state(pair.first, pair.second);
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    //does not work:
    Plugin<OpenList<pair<state_var_t *, short *> > >::register_open_lists();

    parser.add_option<OpenList<pair<state_var_t *, short *> > *>("open");
...
static SearchEngine *_parse_astar(OptionParser &parser) {
    ...
    if (!parser.dry_run()) {
        ...
        OpenList<pair<state_var_t *, short *> > *open = \
            new TieBreakingOpenList<pair<state_var_t *, short *> >(evals, false, false);
...
static SearchEngine *_parse_greedy(OptionParser &parser) {
    ...
    if (!parser.dry_run()) {
        ...
        if ((evals.size() == 1) && preferred_list.empty()) {
            open = new StandardScalarOpenList<pair<state_var_t *, short *> >(evals[0], false);
        } else {
            vector<OpenList<pair<state_var_t *, short *> > *> inner_lists;
            for (int i = 0; i < evals.size(); i++) {
                inner_lists.push_back(
                    new StandardScalarOpenList<pair<state_var_t *, short *> >(evals[i], false));
                if (!preferred_list.empty()) {
                    inner_lists.push_back(
                        new StandardScalarOpenList<pair<state_var_t *, short *> >(evals[i],
                                                                  true));
                }
            }
            open = new AlternationOpenList<pair<state_var_t *, short *> >(
