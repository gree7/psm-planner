...
int EnforcedHillClimbingSearch::step() {
    ...
    for (int i = 0; i < ops.size(); i++) {
        ...
        OpenListEntryEHC entry = make_pair(make_pair(current_node.get_state_buffer(), current_node.get_state_buffer_plans()), make_pair(d, ops[i]));
...
int EnforcedHillClimbingSearch::ehc() {
    while (!open_list->empty()) {
        ...
        State last_parent = search_space.get_node(State(next.first.first, next.first.second)).get_state();
        ...
        if (node.is_new()) {
            ...
            if (h < current_h) {
                ...
                for (int i = 0; i < ops.size(); i++) {
                    ...
                    OpenListEntryEHC entry = make_pair(make_pair(node.get_state_buffer(), node.get_state_buffer_plans()), make_pair(new_d, ops[i]));
