...
LazySearch::LazySearch(const Options &opts)
    : SearchEngine(opts),
      ...
      current_predecessor_buffer_plans(NULL),
...
void LazySearch::generate_successors() {
    ...
    SearchNode sNode = search_space.get_node(current_state);
    state_var_t *current_state_buffer = sNode.get_state_buffer();
    short *current_state_buffer_plans = sNode.get_state_buffer_plans();
    ...
    for (int i = 0; i < operators.size(); i++) {
        ...
        if (new_real_g < bound) {
            ...
            open_list->insert(
                make_pair(make_pair(current_state_buffer, current_state_buffer_plans), operators[i]));
        }
    }
}
...
int LazySearch::fetch_next_state() {
    ...
    current_predecessor_buffer = next.first.first;
    current_predecessor_buffer_plans = next.first.second;
    current_operator = next.second;
    State current_predecessor(current_predecessor_buffer, current_predecessor_buffer_plans);
...
int LazySearch::step() {
    ...
    if (node.is_new() || reopen) {
        ...
        short *dummy_address_plans = current_predecessor_buffer_plans;
        // HACK! HACK! we do this because SearchNode has no default/copy constructor
        if (dummy_address == NULL) {
            dummy_address = const_cast<state_var_t *>(g_initial_state->get_buffer());
            dummy_address_plans = const_cast<short *>(g_initial_state->get_buffer_plans());
        }

        SearchNode parent_node = search_space.get_node(State(dummy_address, dummy_address_plans));
