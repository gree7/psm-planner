...
SearchNode::SearchNode(state_var_t *state_buffer_, short *state_buffer_plans_, SearchNodeInfo &info_, OperatorCost cost_type_)
    : state_buffer(state_buffer_), state_buffer_plans(state_buffer_plans_), info(info_), cost_type(cost_type_) {
}

State SearchNode::get_state() const {
    return State(state_buffer, state_buffer_plans);
}
...
const short *SearchNode::get_parent_buffer_plans() const {
    return info.parent_state_plans;
}
...
void SearchNode::open(int h, const SearchNode &parent_node,
    ...
    info.parent_state_plans = parent_node.state_buffer_plans;
...
void SearchNode::dump() {
    cout << state_buffer << ".." << state_buffer_plans <<": ";
    State(state_buffer, state_buffer_plans).dump_fdr();
    cout << " created by " << info.creating_operator->get_name()
         << " from " << info.parent_state << ".." << info.parent_state_plans << endl;
}
...
SearchNode SearchSpace::get_node(const State &state) {
    ...
    return SearchNode(iter->first.state_data, iter->first.state_data_plans, iter->second, cost_type);
}
...
void SearchSpace::trace_path(const State &goal_state,
    ...
    for (;;) {
        ...
        current_state = StateProxy(const_cast<state_var_t *>(info.parent_state), const_cast<short *>(info.parent_state_plans));
        new_plan.push_back(op->get_name());
    }
    reverse(path.begin(), path.end());
    reverse(new_plan.begin(), new_plan.end());
    g_new_plan.push_back(new_plan);
}

void SearchSpace::dump() {
    int i = 0;
    for (HashTable::iterator iter = nodes->begin(); iter != nodes->end(); iter++) {
        cout << "#" << i++ << " (" << iter->first.state_data << ".." << iter->first.state_data_plans << "): ";
        State(iter->first.state_data, iter->first.state_data_plans).dump_fdr();
        if (iter->second.creating_operator &&
            iter->second.parent_state) {
            cout << " created by " << iter->second.creating_operator->get_name()
                 << " from " << iter->second.parent_state << ".." << iter->second.parent_state_plans << endl;
        } else {
